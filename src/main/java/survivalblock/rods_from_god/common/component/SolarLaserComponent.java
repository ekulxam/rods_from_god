    ...
    public void tick() {
        ...
        if (overheatTicks >= 0) {
            ...
            if (stack.isOf(RodsFromGodItems.SOLAR_PRISM_HEADSET) && RodsFromGodUtil.isAffectedByDaylight(this.obj)) {
                ...
                ticksUsed++;
                if (this.ticksUsed % 60 == 0 && !world.isClient()) {
                    world.playSound(null, this.obj.getX(), this.obj.getY(), this.obj.getZ(), RodsFromGodSoundEvents.SOLAR_PRISM_HEADSET_CHARGE, SoundCategory.PLAYERS, 0.8F, 1.0F);
                }
            } else {
                overheatTicks--;
                ticksUsed = 0;
            ...
            if (overheatTicks >= MAX_OVERHEATING_TICKS) {
                overheatTicks = MAX_OVERHEATING_TICKS;
                if (!world.isClient()) {
