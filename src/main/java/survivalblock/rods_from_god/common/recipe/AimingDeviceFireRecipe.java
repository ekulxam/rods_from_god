...
public class AimingDeviceFireRecipe extends SpecialCraftingRecipe {
    ...
    public boolean matches(CraftingRecipeInput input) { // I don't need world
        ...
        return stacks.size() == 2 && isAimingDeviceAndNetherStar(stacks.getFirst(), stacks.getLast());
    }

    public boolean isAimingDeviceAndNetherStar(ItemStack first, ItemStack second) {
        if (first == null || first.isEmpty() || second == null || second.isEmpty()) {
            return false;
        }
        return (first.isOf(RodsFromGodItems.AIMING_DEVICE) && second.isOf(Items.NETHER_STAR))
                || (second.isOf(RodsFromGodItems.AIMING_DEVICE) && first.isOf(Items.NETHER_STAR));
    }
    ...
    public ItemStack craft(CraftingRecipeInput input, RegistryWrapper.WrapperLookup lookup) {
        ...
        if (stack.getOrDefault(RodsFromGodDataComponentTypes.AIMING_DEVICE_CREATES_FIRE, false)) {
            return ItemStack.EMPTY;
        }
        ItemStack returnStack = stack.copy();
        returnStack.set(RodsFromGodDataComponentTypes.AIMING_DEVICE_CREATES_FIRE, true);
        return returnStack;
    }
